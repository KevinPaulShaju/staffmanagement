...
exports.viewCarer = async (req, res) => {
  try {
    const findCarer = await Carer.find({}).select("-password");

    if (findCarer.length === 0) {
      return res.status(200).json({ message: "No Carer profile to view" });
    } else {
      return res.status(200).json({ messages: findCarer });
    }
  } catch (e) {
    res.status(500).json({ error: e.message });
  }
