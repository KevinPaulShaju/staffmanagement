...
// to add a document
router.post("/add/:staffId",staffDocuments.single("document"),async (req, res)=>{
    ...
    const newDocument = new Document({
        StaffId:_id,
        docType, url:req.file.location ,docNumber
    });
    const savedDocument = await newDocument.save();
    res.status(200).json({message:"Document Inserted",document:savedDocument});
...
router.post("/update/document/:documentId",staffDocuments.single("document"),async (req, res)=>{
    const _id = req.params.documentId
    const findDocument = await Document.findOne({ _id });
    if(!findDocument){
        return res.status(404).json({ error: "Document Not Found"})
    ...
    if(findDocument.url !== null){
        ...
        return res.status(200).json({message: "Document Saved",file:findDocument.url})
    }
